...
.PHONY: help init start stop restart check upload clean up down status wait \
        ...
        start-cls stop-cls start-monitor stop-monitor start-ssh-log stop-ssh-log \
        start-api stop-api status-mcp
...
help:
	...
	@echo "  $(YELLOW)make start-ssh-log$(NC) - 🔎 启动 SSH Log MCP 服务"
	@echo "  $(YELLOW)make stop-ssh-log$(NC)  - 🛑 停止 SSH Log MCP 服务"
...
# 启动 SSH Log MCP 服务
start-ssh-log:
	@echo "$(YELLOW)🔎 启动 SSH Log MCP 服务...$(NC)"
	@if pgrep -f "mcp_servers/ssh_log_server.py" > /dev/null 2>&1; then \
		echo "$(GREEN)✅ SSH Log MCP 服务已经在运行中$(NC)"; \
	else \
		echo "$(YELLOW)📦 正在启动 SSH Log MCP 服务（后台运行）...$(NC)"; \
		nohup .venv/bin/python mcp_servers/ssh_log_server.py > mcp_ssh_log.log 2>&1 & \
		echo $$! > mcp_ssh_log.pid; \
		sleep 2; \
		if pgrep -f "mcp_servers/ssh_log_server.py" > /dev/null 2>&1; then \
			echo "$(GREEN)✅ SSH Log MCP 服务启动成功$(NC)"; \
			echo "$(YELLOW)   PID: $$(cat mcp_ssh_log.pid)$(NC)"; \
			echo "$(YELLOW)   URL: http://127.0.0.1:8005/mcp$(NC)"; \
			echo "$(YELLOW)   日志: mcp_ssh_log.log$(NC)"; \
		else \
			echo "$(RED)❌ SSH Log MCP 服务启动失败$(NC)"; \
			echo "$(YELLOW)请检查日志: tail -f mcp_ssh_log.log$(NC)"; \
		fi; \
	fi

# 停止 SSH Log MCP 服务
stop-ssh-log:
	@echo "$(YELLOW)🛑 停止 SSH Log MCP 服务...$(NC)"
	@if [ -f mcp_ssh_log.pid ]; then \
		pid=$$(cat mcp_ssh_log.pid); \
		if ps -p $$pid > /dev/null 2>&1; then \
			kill $$pid; \
			echo "$(GREEN)✅ SSH Log MCP 服务已停止 (PID: $$pid)$(NC)"; \
		else \
			echo "$(YELLOW)⚠️  进程不存在 (PID: $$pid)$(NC)"; \
		fi; \
		rm -f mcp_ssh_log.pid; \
	else \
		echo "$(YELLOW)⚠️  未找到 mcp_ssh_log.pid 文件$(NC)"; \
		pkill -f "mcp_servers/ssh_log_server.py" 2>/dev/null && \
			echo "$(GREEN)✅ 已停止所有 SSH Log MCP 进程$(NC)" || \
			echo "$(YELLOW)⚠️  没有运行中的 SSH Log MCP 进程$(NC)"; \
	fi
...
status-mcp:
	...
	@echo ""
	@echo "$(CYAN)SSH Log MCP 服务:$(NC)"
	@if pgrep -f "mcp_servers/ssh_log_server.py" > /dev/null 2>&1; then \
		pid=$$(pgrep -f "mcp_servers/ssh_log_server.py"); \
		echo "  状态: $(GREEN)运行中$(NC)"; \
		echo "  PID: $$pid"; \
...
clean:  ## 清理临时文件
	...
	rm -f mcp_ssh_log.pid mcp_ssh_log.log
